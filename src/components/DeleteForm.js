...
export default function DeleteForm({ handleDeleteRecipe, visible }) {
    return (
        <div id="delete-form" className={`${visible ? "visible" : ""} card`}>
            ...
            <hr/>
            <div id="buttons">
                <span id="delete-form-cancel" className="primary-btn" onClick={() => handleDeleteRecipe(false)}>Cancel</span>
                <span id="delete-form-delete" className="primary-btn" onClick={() => handleDeleteRecipe(true)}>Delete</span>
            </div>
        </div>
    );
}
